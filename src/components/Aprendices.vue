...
async function fetchData() {
  const response = await fetch('http://localhost:5173/api/Ficha/ListarTodo', {
    headers: {
...
async function Activar(id) {
  console.log(id);
  try {
    inf = await axios.put(`http://localhost:5173/api/Aprendiz/Desactivar/${id}`)
    traer();
  } catch (error) {
    console.log(error);
  }
...
async function Desactivar(id) {
  console.log(id);
  try {
    inf = await axios.put(`http://localhost:5173/api/Aprendiz/Activar/${id}`)
    traer();
  } catch (error) {
    console.log(error);
  }
