...
export const UseFichaStore = defineStore("ficha", () => {
    ...
    const listarFicha = async () => {
        try {
            let inf = await axios.get('http://localhost:5173/api/Ficha/ListarTodo',{
                headers:{
    ...
    const crearFicha = async (codigo, nombre) => {
        loading.value=true
        try {
            let inf = await axios.post('http://localhost:5173/api/Ficha/crear',{
                Codigo: codigo,
                Nombre: nombre
            },{
                headers:{
    ...
    const EditarFicha = async (id,nombre,codigo) => {
        loading.value=true
        try {
            let inf = await axios.put(`http://localhost:5173/api/Ficha/Actualizar/${id}`,{
                Nombre:nombre,
                Codigo:codigo,
            },{
                headers:{
