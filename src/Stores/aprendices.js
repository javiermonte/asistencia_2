...
export const UseAprendizStore = defineStore("aprendiz", () => {
    ...
    const listarAprediz = async () => {
        ...
        try {
            let res = await axios.get('http://localhost:5173/api/Aprendiz/ListarTodo', {
                headers:{
    ...
    const registrarAprendiz = async (nombre, telefono, documento, email, ficha) => {
        loading.value=true
        try {
            let inf = await axios.post('http://localhost:5173/api/Aprendiz/Insertar', {
                Nombre: nombre,
                Telefono: telefono,
                Documento: documento,
                Email: email,
    ...
    const editarAprendiz = async (id, nombre, telefono, documento, email, ficha) => {
        try {
            let inf = await axios.put(`http://localhost:5173/api/Aprendiz/Actualizar/${id}`, {
                Nombre: nombre,
                Telefono: telefono,
                Documento: documento,
                Email: email,
